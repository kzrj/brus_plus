...
from stock.models import Shift, Lumber, LumberRecord, Shop, Sale
...
class LumberStockViewTest(APITestCase):
    def setUp(self):
        ...
        self.shop2 = Shop.objects.filter(name='shop2').first()
        ...
        self.seller1.account.can_see_shop_stock.add(self.shop, self.shop2)
        self.seller2 = User.objects.get(username='seller1')
        self.seller2.account.can_see_shop_stock.add(self.shop2)
...
class ShiftStockViewTest(APITestCase):
    def setUp(self):
        ...
        self.manager1.account.can_see_shop_shift.add(self.shop)

        self.manager2 = User.objects.get(username='manager2')
        self.manager2.account.can_see_shop_shift.add(self.shop2)

        self.capo1 = User.objects.get(username='capo1')
        self.capo1.account.can_see_shop_shift.add(self.shop, self.shop2)
    ...
    def test_get_shop_shifts(self):
        self.client.force_authenticate(user=self.manager1)
        response = self.client.get(f'/api/common/shifts/?shop={self.shop.pk}')
        ...
        response = self.client.get(f'/api/common/shifts/?shop={self.shop2.pk}')
        self.assertEqual(response.status_code, 200)
        self.client.logout()

    def test_get_shop_shifts_permissions(self):
        self.client.force_authenticate(user=self.manager1)
        response = self.client.get(f'/api/common/shifts/?shifts={self.shop2.pk}')
        self.assertEqual(response.status_code, 403)

        response = self.client.get(f'/api/common/shifts/?shop=335')
        self.assertEqual(response.status_code, 403)
        self.client.logout()
    ...
    def test_get_shop_shifts_filter(self):
        self.client.force_authenticate(user=self.manager1)
        date1 = datetime.date.today()
        date2 = date1 - datetime.timedelta(days=1)
        date3 = date1 + datetime.timedelta(days=2)

        response = self.client.get(f'/api/common/shifts/?shop={self.shop.pk}&date_after=\
...
class SalesStockViewTest(APITestCase):
    def setUp(self):
        ...
        self.shop = Shop.objects.filter(name='batoshop').first()
        self.shop2 = Shop.objects.filter(name='shop2').first()
        ...
        self.seller1 = User.objects.get(username='sergei')
        self.seller1.account.can_see_shop_sales.add(self.shop, self.shop2)

        self.seller2 = User.objects.get(username='seller1')
        self.seller2.account.can_see_shop_sales.add(self.shop2)

        self.kladman = User.objects.get(username='kladman')

        self.manager1 = User.objects.get(username='manager1')
        self.manager1.account.can_see_shop_sales.add(self.shop, self.shop2)

        self.manager2 = User.objects.get(username='manager2')
        self.manager2.account.can_see_shop_sales.add(self.shop2)

        self.capo1 = User.objects.get(username='capo1')
        self.capo1.account.can_see_shop_sales.add(self.shop)
        ...
        data_list2 = {
            'lumbers': [
                {'lumber': self.brus1, 'quantity': 20, 'shop_price': 12000, 
                    'selling_price': 12500,'selling_total_cash': 7500, 'calc_type': 'exact'},
                {'lumber': self.doska25_18, 'quantity': 60, 'shop_price': 7000,
        ...
        data_list3 = {
            'lumbers': [
                {'lumber': self.brus1, 'quantity': 30, 'shop_price': 12000, 'selling_price': 12500,
                    'selling_total_cash': 7500, 'calc_type': 'exact'},
                {'lumber': self.china_brus1, 'quantity': 30, 'shop_price': 15000,
                    'selling_price': 15000, 'selling_total_cash': 19010, 'calc_type': 'china'},
            ],
            'loader': True,
            'seller': self.seller2,
    ...
    def test_get_shop_sales(self):
        self.client.force_authenticate(user=self.manager1)
        response = self.client.get(f'/api/common/sales/?shop={self.shop.pk}')
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.get(f'/api/common/sales/?shop={self.shop2.pk}\
    ...
    def test_get_shop_sales_filter(self):
        self.client.force_authenticate(user=self.seller2)
        response = self.client.get(f'/api/common/sales/?shop={self.shop2.pk}\
    ...
    def test_get_shop_sales_permissions(self):
        self.client.force_authenticate(user=self.capo1)       
        response = self.client.get(f'/api/common/sales/?shop={self.shop2.pk}')
        self.assertEqual(response.status_code, 403)
        self.client.logout()
...
class CashRecordsViewTest(APITestCase):
    def setUp(self):
        ...
        self.manager1 = User.objects.get(username='manager1')
        self.manager1.account.can_see_shop_cash.add(self.shop)
        self.manager2 = User.objects.get(username='manager2')
        self.manager1.account.can_see_shop_cash.add(self.shop2)

        self.capo1 = User.objects.get(username='capo1')
        self.capo1.account.can_see_shop_cash.add(self.shop, self.shop2)

        self.capo2 = User.objects.get(username='capo2')
        self.capo2.account.can_see_shop_cash.add(self.shop2)
    ...
    def test_get_shop_cash(self):
        self.client.force_authenticate(user=self.capo1)       
        response = self.client.get(f'/api/common/cash/?shop={self.shop2.pk}')
        self.assertEqual(response.data['count'], 1)
        response = self.client.get(f'/api/common/cash/?shop={self.shop.pk}')
        self.assertEqual(response.data['count'], 1)
        self.client.logout()
