...
from stock.models import Shift, Lumber, LumberRecord, Sale, Shop, ReSaw
...
class SaleServisesTest(TransactionTestCase):
    ...
    def test_create_resaw(self):
        ...
        self.assertEqual(resaw.lumber_in.lumber, self.brus1)
        self.assertEqual(resaw.lumber_out.lumber, self.doska1)
        self.assertEqual(resaw.lumber_in.quantity, 10)
        self.assertEqual(resaw.lumber_out.quantity, 44)

        self.assertEqual(resaw.lumber_in.volume, 0.6)

        self.assertEqual(LumberRecord.objects.all().count(), 2)
        self.assertEqual(LumberRecord.objects.all().first().record_type, 'resaw_record')
