...
#     def test_calc_total_income_volume_by_shop_by_lumber(self):
...
#         total_income_volume = LumberRecord.objects.calc_total_income_volume_by_shop_by_lumber(
#             lumber=self.brus1, shop=self.shop)[0]['total_income_volume']

#         self.assertEqual(total_income_volume, 0.6)

#     def test_calc_total_outcome_volume_by_shop_by_lumber(self):
#         sale_data_list = [
...
#          volume=10, initiator=self.ramshik1, shop=self.shop)

#         total_outcome_volume = LumberRecord.objects.calc_total_outcome_volume_by_shop_by_lumber(
#             lumber=self.brus1, shop=self.shop)[0]['total_outcome_volume']

#         self.assertEqual(total_outcome_volume, 0.6)

#     def test_lumber_add_shop_current_stock(self):
#         employees = [self.ramshik1.account, self.ramshik2.account, self.ramshik3.account]
#         shift_data_list = [
#          {'lumber': self.brus1, 'quantity': 10, 'volume_total': 0.6, 'rate': 600, 'cash': 360 },
#          {'lumber': self.brus2, 'quantity': 10, 'volume_total': 0.4, 'rate': 600, 'cash': 240 },
...
#         lumbers = Lumber.objects.all().add_shop_current_stock(shop=self.shop)
